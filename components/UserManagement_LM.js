...
const TxtField = (props) => {
  const userRecords = useRecoilValue(SortedRecord_LMSelector);
...
const UserManagement = () => {
  const userRecords = useRecoilValue(Records_LMAtom);
  ...
  const HandleOnclick = (value) => () => {
    mongoPost(
      modalTitle === "Remove" ? "/deleteRecord_LM" : "/addRecords_LM",
      value,
    );
    setOpenModal(false);
    setValue([]);
