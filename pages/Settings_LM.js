...
import { EdittedQnBnk_LMSelector } from "../state/quiz_LMState";
...
export default function Settings() {
  const [rows, setRows] = useRecoilState(EdittedQnBnk_LMSelector);
  ...
  const handleSaveClick = (id) => () => {
    ...
    if (_.isBoolean(isNew)) {
      //console.log("new");
      mongoPost("/addQuiz_LM", body);
    } else {
      //console.log("existing");
      const { _id, ...others } = body;
      mongoPost(`/editQuiz_LM?id=${_id}`, others);
    }
  };

  const handleDeleteClick = (id) => () => {
    setRows(rows.filter((row) => row._id !== id));
    mongoPost(`/deleteQuiz_LM?id=${id}`);
  };
